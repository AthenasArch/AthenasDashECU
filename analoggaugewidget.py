...
class AnalogGaugeWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        QFontDatabase.addApplicationFont(os.path.join(os.path.dirname(
            __file__), 'fonts/ds_digital/DS-DIGIB.ttf'))
        ...
        self.value_fontname = "DS-Digital"
    ...
    def updateValue(self, value, mouse_controlled=False):
        ...
        #
        # if mouse_controlled:
        #     self.signals.valueChanged.emit(int(value))
    ...
    def create_fine_scaled_marker(self):
        # Criar um novo objeto QPainter.
        my_painter = QPainter(self)
        # Ativar o anti-aliasing (suavização de bordas).
        my_painter.setRenderHint(QPainter.Antialiasing)
        # Mover o ponto de origem para o centro do widget.
        my_painter.translate(self.width() / 2, self.height() / 2)
        # Definir a cor da linha.
        my_painter.setPen(self.fineScaleColor)
        # Rotacionar o sistema de coordenadas.
        my_painter.rotate(self.scale_angle_start_value - self.angle_offset)
        # Calcular o tamanho de cada passo.
        steps_size = (float(self.scale_angle_size) / float(self.scalaCount * self.scala_subdiv_count))
        # Calcular o início e o fim de cada linha.
        scale_line_outer_start = self.widget_diameter // 2
        ...
        for i in range((self.scalaCount * self.scala_subdiv_count) + 1):
            ...
            my_painter.rotate(steps_size)
        # Limpar o QPainter quando terminar.
        my_painter.end()
...
# Sim, esses métodos são responsáveis pelo tratamento dos eventos de mouse.

# setMouseTracking(flag): Este método é usado para definir se o rastreamento 
#   do mouse está ativado. Se o rastreamento do mouse estiver ativado, o widget 
#   receberá eventos de movimento do mouse mesmo quando nenhum botão estiver 
#   pressionado. Se o rastreamento do mouse estiver desativado (o padrão), o 
#   widget só receberá eventos de movimento do mouse quando um botão do mouse 
#   estiver pressionado.

# mouseReleaseEvent(QMouseEvent): Este método é chamado quando um botão do mouse 
#   é liberado. Aqui, ele está mudando a cor da agulha para NeedleColorReleased 
#   e atualizando a exibição.

# leaveEvent(event): Este método é chamado quando o cursor do mouse deixa o widget.
#   Aqui, ele também está mudando a cor da agulha para NeedleColorReleased e 
#   atualizando a exibição.

# mouseMoveEvent(event): Este método é chamado sempre que o cursor do mouse se move 
#   sobre o widget (se o rastreamento do mouse estiver ativado). Aqui, ele está 
#   calculando o valor correspondente à posição do cursor do mouse e atualizando a exibição.

# Quanto à sua pergunta sobre como usar um timer em vez do mouse, você poderia usar um 
#   QTimer para isso. Aqui está um exemplo básico de como você poderia fazer isso: